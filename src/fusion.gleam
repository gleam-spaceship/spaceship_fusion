...
pub fn main() {
  ...
  case args {
    ["build"] -> do_build(find_project_root())
    ["build", "--watch"] -> do_build(find_project_root())
    ["build", path] -> do_build(path)
    ["scan"] -> do_scan(find_project_root())
    ["scan", path] -> do_scan(path)
    _ -> print_help()
  }
}

fn do_build(root: String) {
...
fn print_help() {
  io.println("fusion - Write JS classes in Gleam\n")
  io.println("Commands:")
  io.println("  build            Full pipeline → build/dist/")
  io.println("  build --watch    Full pipeline (watch mode TBD)")
  io.println("  build <path>     Build specific project path")
  io.println("  scan             List class .gleam files")
  io.println("  scan <path>      Scan specific project path")
}
